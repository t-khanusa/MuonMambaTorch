...
from dataset import MMAct_Dataset
import copy
import time
import math
...
DATA
'''
sensor_fold = "/project/khanhnt/HAR_dataset/MMAct/sensor_combine"
modulation_dict = {}

label_dict = {'exiting': 9, 'setting_down': 23, 'carrying_light': 3, 'entering': 8, 'waving_hand': 35, 'crouching': 6, 'looking_around': 14, 'standing': 26, 'throwing': 30, 'picking_up': 16, 'carrying_heavy': 2, 'talking_on_phone': 29, 'jumping': 11, 'fall': 10, 'talking': 28, 'pointing': 19, 'closing': 5, 'running': 22, 'carrying': 1, 'pulling': 20, 'pushing': 21, 'transferring_object': 31, 'checking_time': 4, 'loitering': 13, 'kicking': 12, 'walking': 34, 'opening': 15, 'using_phone': 33, 'drinking': 7, 'sitting': 24, 'pocket_out': 18, 'pocket_in': 17, 'using_pc': 32, 'sitting_down': 25, 'standing_up': 27, 'Carrying_light': 0}
sorted_label_dict = dict(sorted(label_dict.items(), key=lambda item: item[1]))
modulation_list = list(sorted_label_dict.keys())

train_x, train_y = [], []
val_x, val_y = [], []

# /hdd1/khanhnt/sensor_dataset/subject1/scene1/session1/checking_time.npy
for sensor in glob.glob(os.path.join(sensor_fold, '*', '*', '*', '*.csv')):
    subject = sensor.split("/")[6][7:]

    if int(subject) < 17:
        train_x.append(sensor)
        train_y.append(label_dict[sensor.split("/")[-1][:-4]])
    else:
        val_x.append(sensor)
        val_y.append(label_dict[sensor.split("/")[-1][:-4]])
...
train_dataset = MMAct_Dataset(train_x, train_y)
val_dataset = MMAct_Dataset(val_x, val_y)
...
parser.add_argument('--d_state', action="store", dest='d_state', default=32)
parser.add_argument('--num_classes', action="store", dest='num_classes', default=36)
parser.add_argument('--input_channels', action="store", dest='input_channels', default=12)
parser.add_argument('--n_layers', action="store", dest='n_layers', default=2)
parser.add_argument('--momentum_beta', action="store", dest='momentum_beta', default=0.99)
parser.add_argument('--momentum_alpha', action="store", dest='momentum_alpha', default=0.6)
...
print(f"Test Accuracy (using val set): {test_acc:.4f}, Test Loss: {test_loss:.4f}")
